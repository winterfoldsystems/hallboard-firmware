...
python3 firmware/sim/tools/night_palette.py    # regenerates hb_tokens.h's NIGHT block
python3 firmware/sim/tools/temp_scale.py       # regenerates hb_temp_scale.h
firmware/sim/.venv/bin/python3 firmware/sim/tools/icons.py   # regenerates hb_icons.h
```
...
`firmware/hb_icons.h` (the weather face's icons, both the three cards and the hourly strip) is
generated by `tools/icons.py` from the SVGs vendored into `firmware/icons/meteocons/` (source,
licence and the HallBoard-name-to-file mapping in that directory's own README). It needs a local
SVG rasteriser: cairosvg and Pillow, in a venv under `firmware/sim/.venv/` (gitignored, not a
build dependency of the device or of `hbsim`):

```
cd firmware/sim
python3 -m venv .venv
.venv/bin/pip install cairosvg Pillow
.venv/bin/python3 tools/icons.py
```

cairosvg pulls in a system Cairo; `brew install cairo` first if the pip install fails to build it.
...
| Name | What it shows |
|---|---|
| `clock` | The clock page of a paired board: the date above the numerals, temperature, conditions and wind below |
